...
namespace GameDev2D
{
	...
	void BatchRenderer::RenderLineStrip(std::vector<Vector2>& points, const Color& color, float lineWidth, Vector2 pos, float angle)
	{
		if (m_ActiveRenderer->GetType() != RendererType::Line)
		{
			SwitchRenderer(RendererType::Line);
		...
		m_LineRenderer->RenderLineStrip(points, color, lineWidth, pos, angle);
	}
