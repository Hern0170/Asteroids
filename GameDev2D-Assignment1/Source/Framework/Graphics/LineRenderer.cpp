...
namespace GameDev2D
{
	...
	void LineRenderer::RenderLineStrip(std::vector<Vector2>& points, const Color& color, float lineWidth, Vector2 pos, float angle)
	{
		...
		if (points.size() > 1)
		{
			...
			float angleR = Math::DegreesToRadians( angle );
			std::vector<Vector2> transformedPoints;
			for (unsigned int i = 0; i < points.size(); i++)
			{
				transformedPoints.push_back( pos + Vector2( points[i].x * cos(angleR) - points[i].y * sin(angleR), points[i].x * sin(angleR) + points[i].y * cos(angleR) ) );
			}
